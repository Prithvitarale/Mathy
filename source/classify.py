...
class classify:
    ...
    def process_data(self, data_dir_without_star, concept):
        ...
        jj=0
        for j, dir_path in enumerate(glob.glob(data_dir)): #mathy_pos, mathy_neg
            print(dir_path)
            if concept in dir_path:
                print("processing data")
                print(dir_path)
                dir_path = os.path.join(dir_path, "*")
                for k, dirr in enumerate(glob.glob(dir_path)): #test_neg, train
    ...
    def load_embeddings(self, data_dir, concept):
        ...
        concept_path = os.path.join(data_dir)
        embeddings_path = os.path.join(concept_path, "embeddings"+concept)
    ...
    def save_model(self, model_name):
        pickle.dump(self.model, open('concept_models/' + model_name + '.pkl', 'wb'))
        print('model saved')
    ...
    def learning_curve_new(self, x, y, model, output, concept):
        ...
        #TODO: train_y_subset is unevenly split?
        ...
        seeds = range(0, 25)
        # seeds = range(0, 10)
        # keep sampling until enough samples are obtained
        min_files = 8
...
def main():
    ...
    parser.add_argument("--save_concept_model", required=False, action="store_true")
    ...
    if args.save_concept_model:
        c.load_embeddings(args.data_dir, args.concept)
        c.train(model)
        c.save_model(args.concept + "_model")
...
# collect papers for {guidelines_pos, qualitative_pos, sentiment} concepts
...
# output file for lc
# sentiment -
# guidelines_pos, principles, ben shneiderman,
